...
public class User implements IData {	
	 ...
	 */
	public List<Project> getProjects() throws SQLException, InvalidProjectTypeException 
    {
		return Projects.getProjectsByResearcher(this.id);
	}
	
	
	public List<Project> getProjectsWithOverdueReports() throws InvalidProjectTypeException, SQLException {
	    List<Project> userProjects = getProjects();
	    Iterator<Project> projIter = userProjects.iterator();
	    while(projIter.hasNext()) {
	        Project proj = projIter.next();
	        if(proj instanceof Collaboration) {
	            // keep the project in the list if a report is overdue
	            if(((Collaboration)proj).isProgressReportOverdue())
	                continue;
	            // otherwise remove it
	            else
	                projIter.remove();
	...
	public List<Collaboration> getCollaborationsToReview() throws SQLException {
	    return Projects.getCollaborationsByReviewer(this.id);
	}
