...
public class ProjectInstrumentUsageDAO {
	...
	public List<UsageBlock> getUsageBlocksForProject(int projectId, int instrumentId, int paymentMethodId, Date startDate, Date endDate) throws SQLException {
        ...
        if(instrumentId != 0)
        {
            buf.append(" AND ins.id = ").append(instrumentId);
        }
        if(startDate != null)
        {
            buf.append(" AND insUsg.startDate >= '").append(dateFormat.format(startDate)).append("'");
        }
        if(endDate != null)
        {
            buf.append(" AND endDate <= '").append(dateFormat.format(endDate)).append("'");
        }

        buf.append(" AND r.researcherID=proj.projectPI ");
        buf.append(" AND ins.id=insUsg.instrumentID ");
        buf.append(" AND insUsg.projectID=").append(projectId);
	...
	public List<UsageBlockBase> getUsageBlocksForProjectInRange(int projectId, Date startDate, Date endDate) throws SQLException {
        ...
        buf.append(" WHERE projectID=").append(projectId);
        if(startDate != null) {
            buf.append(" AND startDate >= '").append(dateFormat.format(startDate)).append("'");
        }
        if(endDate != null)
            buf.append(" AND endDate <= '").append(dateFormat.format(endDate)).append("'");
        String sql = buf.toString();
