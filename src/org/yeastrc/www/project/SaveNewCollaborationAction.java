...
public class SaveNewCollaborationAction extends Action {
	...
	throws Exception {
        ...
        // ASSIGN reviewer(s)
        List<Researcher> reviewers = null;
        if (isTech){
            // We don't have tech project for UWPR.  isTech should never be true.
        } else {
            // Assign a reviewer to the project
            try {
                reviewers = ReviewerAssignmentUtils.assignReviewer((Collaboration) project);
            }
		...
		if (sendEmail) {
			emailCollaborators(request, (Collaboration) project, user, reviewers);
		}
    ...
    private void emailCollaborators(HttpServletRequest request, Collaboration collab, User user, List<Researcher> reviewers) {
        try {
        	...
        	String[] groups = collab.getGroupsArray();
        	for (int i = 0; i < groups.length; i++) {
        	...
        	// set the subject
        	message.setSubject("UWPR - New Collaboration Request");
...
//        	    text += "The following reviewers have been assigned to the project: \n";
//        	    for(Researcher reviewer: reviewers) {
//        	        text += reviewer.getFirstName()+" "+reviewer.getLastName()+", E-mail: "+reviewer.getEmail()+"\n";
//        	    }
//        	}
//        	text += "\n\n";
        	
        	if (collab.getPI() != null)
        		text += "PI: " + collab.getPI().getListing() + "\n\n";

        	//text += "Groups: " + project.getGroupsString() + "\n\n";
        	text += "Title: " + collab.getTitle() + "\n\n";
        	text += "Abstract: " + collab.getAbstract() + "\n\n";
        	text += "Scientific Question: " + collab.getScientificQuestion() + "\n\n";

        	text += "MS Runs requested:\n";
        	text += "\tLTQ:\t" + collab.getLtqRunsRequested() + "\n";
        	text += "\tLTQ-ETD:\t" + collab.getLtq_etdRunsRequested() + "\n";
        	text += "\tLTQ-Orbitrap:\t" + collab.getLtq_orbitrapRunsRequested() + "\n";
        	text += "\tLTQ-FT:\t" + collab.getLtq_ftRunsRequested() + "\n";
        	text += "\tTSQ-Access:\t" + collab.getTsq_accessRunsRequested() + "\n";
        	text += "\tTSQ-Vantage:\t" + collab.getTsq_vantageRunsRequested() + "\n\n";
        	
        	text += "Fragmentation type(s) requested: ";
        	if (collab.getFragmentationTypesArray() == null || collab.getFragmentationTypesArray().length == 0) 
        	    text+= "\n\n";
        	else
        	    text += org.apache.commons.lang.StringUtils.join( collab.getFragmentationTypesArray(), ", " ) + "\n\n";

        	text += "Database searched at UWPR: " + collab.isDatabaseSearchRequested() + "\n\n";
        	text += "Mass Spec. analysis by UWPR personnel: " + collab.isMassSpecExpertiseRequested() + "\n\n";
        	
        	text += "Comments: " + collab.getComments() + "\n\n";
